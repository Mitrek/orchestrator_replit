...
const alpha = Math.max(0, Math.min(1, parseFloat(getArg("alpha", "0.6"))));
const circlesAlpha = Math.max(0, Math.min(1, parseFloat(getArg("circles", "0.85"))));
...
async function collectDomCandidates(page) {
  return await page.evaluate(() => {
    ...
    return { cands, viewport: { w: vpW, h: vpH } };
  });
}
...
// ---------------- Point Verification and Cleaning ----------------
async function verifyAndCleanPoints(viewportBuf, vpW, vpH, rawPoints, domCands = []) {
  ...
  const findSnapTarget = (p) => {
    ...
    if (!best) return null;
    const r = best.rExp;
    const thresh = Math.max(0.12, 0.6 * r.w, 0.35 * r.h);
...
// ---------------- Heatmap Rendering (GPT Art Style) ----------------
async function renderOverlay(fullJpgPath, pointsJson, viewportPxHeight, outHeatPngPath, outOverlayPath, heatAlpha, circlesAlpha) {
  ...
  // Green/Red baseline (Green above fold, Red below)
  ...
  for (let y = 0; y < GH_FULL; y++) {
    const v = (y < GH_FOLD) ? 1.0 : 0.0; // 1.0 = green above fold, 0.0 = red below
    const o = y * GW_FULL;
  ...
  for (const p of pts) {
    ...
    const cy = Math.round(clamp01(p.y) * (GH_FOLD - 1));
  ...
  // Convert grid to PNG heatmap (Green/Red instead of Red/Blue)
  const heat = new PNG({ width: GW_FULL, height: GH_FULL });
  for (let y = 0; y < GH_FULL; y++) {
    for (let x = 0; x < GW_FULL; x++) {
      const v = grid[y * GW_FULL + x]; // 0..1
      const G = Math.round(255 * v);      // Green for above fold
      const R = Math.round(255 * (1 - v)); // Red for below fold
      const idx = (y * GW_FULL + x) * 4;
      heat.data[idx] = R;     // Red channel
      heat.data[idx+1] = G;   // Green channel
      heat.data[idx+2] = 0;   // Blue channel
      heat.data[idx+3] = 255; // Alpha
    }
  }
...
async function drawCirclesPng(GW, GH, pts, alpha, viewportPxHeight) {
  ...
  const R = 255, G = 255, B = 0, A = Math.round(255 * alpha); // Yellow circles
  const foldY = viewportPxHeight;

  for (const p of pts) {
    const cx = Math.round(clamp01(p.x) * (GW - 1));
    const cy = Math.round(clamp01(p.y) * (foldY - 1));
...
async function main() {
  ...
  try {
    ...
    console.log("Collecting DOM candidates...");
    const domInfo = await collectDomCandidates(page);
    const domCands = Array.isArray(domInfo?.cands) ? domInfo.cands : [];
    try { 
      await fs.writeFile(outJpg.replace(/\.jpe?g$/i, "") + ".dom.candidates.json", JSON.stringify(domCands, null, 2), "utf-8"); 
    } catch {}
    ...
    // Generate eye-catchers with OpenAI
    console.log("OpenAI: Generating eye-catchers...");
    ...
    // Verify and clean points
    const vpMeta = await sharp(viewportCropBuf).metadata();
    console.log("OpenAI: Verifying crops...");
    ...
    console.log("Rendering heatmap overlay...");
