          ...
          export default function ApiKeys({ user }: { user: User | null }) {
            ...
            const createApiKeyMutation = useMutation({
              ...
              onSuccess: (data) => {
                ...
                // NEW: open the one-time modal if we received the plaintext key
                if (data?.apiKey) {
                ...
                toast({
                  ...
                  description: `New API key "${data?.name ?? form.getValues("name")}" has been created successfully.`,
                });
              },
              onError: (error: any) => {
                toast({
            ...
            return (
              <Layout user={user}>
                ...
                {/* NEW: One-time API key dialog */}
                <Dialog
                  open={showKeyDialog}
                  onOpenChange={(open) => {
                    if (!open) setApiKeyPlain(null); // enforce one-time visibility
                ...
                >
                  <DialogContent className="sm:max-w-lg">
                    ...
                    <DialogFooter className="mt-4">
                      <Button onClick={() => setShowKeyDialog(false)} data-testid="close-one-time-key">
                        I saved it
                      </Button>
                    </DialogFooter>
                  </DialogContent>
